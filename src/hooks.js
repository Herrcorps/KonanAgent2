...
function load() {
  ...
  if (!existsSync(HOOKS_PATH)) {
    return { hooks: [], ctas: [], rules: { doubleDown: [], testing: [], dropped: [] } };
  }
  try {
    return JSON.parse(readFileSync(HOOKS_PATH, 'utf-8'));
  } catch {
    return { hooks: [], ctas: [], rules: { doubleDown: [], testing: [], dropped: [] } };
  }
}
...
export function recordPost({ postId, hook, cta, date, views = 0, likes = 0, comments = 0, shares = 0 }) {
  ...
  if (cta) {
    ...
    if (ctaEntry) {
      ctaEntry.timesUsed += 1;
      ctaEntry.totalViews += views;
    } else {
      data.ctas.push({ text: cta, timesUsed: 1, totalViews: views });
    }
  }
...
export function applyDecisionRules() {
  ...
  // Group hooks by text to detect repeated low performance
  const hooksByText = {};
