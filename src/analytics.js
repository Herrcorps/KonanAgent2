...
export function diagnosePost(postData) {
  ...
  if (highViews && highEngagement) {
    return {
      ...
      emoji: '🟢',
      label: 'Scale It',
      message: 'This is working. Make 3 variations of this hook immediately. Test different posting times.',
      views,
  ...
  if (highViews && !highEngagement) {
    return {
      ...
      emoji: '🟡',
      label: 'Fix the CTA',
      message: 'People are seeing it but not engaging. Try different CTAs on slide 6. Check if the content delivers on the hook promise.',
      views,
      engagement: (engagement * 100).toFixed(2) + '%',
    };
  }
  if (!highViews && highEngagement) {
    return {
      ...
      emoji: '🟡',
      label: 'Fix the Hooks',
      message: 'People who see it love it, but not enough people are seeing it. Test radically different hooks and slide 1 images.',
      views,
      engagement: (engagement * 100).toFixed(2) + '%',
    };
  }
  return {
    ...
    emoji: '🔴',
    label: 'Full Reset',
    message: 'Neither the hook nor the content is working. Try a completely different format, audience angle, or hook category.',
    views,
    engagement: (engagement * 100).toFixed(2) + '%',
  };
}
...
export function generateReport(posts) {
  ...
  for (const p of analyzed) {
    counts[p.diagnosis.status] = (counts[p.diagnosis.status] || 0) + 1;
  }
  ...
  switch (dominant) {
    case 'SCALE':
      recommendation = 'Most posts are performing well. Double down on winning hooks and test variations.';
      break;
    case 'FIX_CTA':
      recommendation = 'Views are good but engagement is low. Focus on improving CTAs and slide 6 content.';
      break;
    case 'FIX_HOOKS':
      recommendation = 'Content quality is high but reach is low. Experiment with new hook styles and posting times.';
      break;
    default:
      recommendation = 'Performance is below baseline. Try new formats, different audience angles, and fresh hook categories.';
  }
...
export function suggestNextActions(posts) {
  ...
  if (actions.length === 0) {
    actions.push(report.recommendation);
  }
